...
mit dem Hauptspeicher abzugleichen. Also einen globalen `flush` zu vollführen. Auf der Gegenseite, muss ein Thread, der
lesend auf eine `volatile` Variable zugreift, diese immer aus dem Hauptspeicher per `refresh` aktualisieren.

*Achtung: Geflusht werden dabei lediglich primitive Werte und Objektreferenzen. Nicht die Objekte selbst!* Diese müssen
selbst für ihre Konsistenz sorgen. Auch sollte man beachten, dass ein kompletter Abgleich mit dem Hauptspeicher sehr

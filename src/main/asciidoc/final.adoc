...
* `flag`: Die JVM garantiert uns für ein Boolean immer einen atomaren Zugriff. Soweit so gut.
        ...
        geflusht wurde. -> _Sequential Consistency_
* `read`: Die Methode geht davon aus, dass zuert die long-Variable und dann erst das flag auf true gesetzt wurde.
          ...
          Anweisungen gedreht werden -> _Reordering_

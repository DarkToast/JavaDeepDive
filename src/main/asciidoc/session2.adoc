...
alle Objekte aufnehmen, die irgendwie von `Object` abgeleiten haben.
...
Das es sich dabei nun um zwei komplett verschiedene Typen handelt, können wir die Referenzen auch nicht kombinieren. Eine
`List<String>` ist eben keine `List<Integer>`.

Denn auch wenn die Parametertypen von zwei Referenzen in einer gemeinsamen Hierachie existieren, so sind es die
...
Eine `List<String>` ist keine `List<Object>`, obwohl ein `String` ein `Object` ist! Das trotzdem eine Hierarchie
...
Genrics sind nicht nur Klassen vorbehalten. Man kann in Java auch generische Methoden erzeugen. Also Methode, die
beim Aufruf einen Typbezeichner erhalten, den sie in ihrer internen Logik benutzen sollen.

Im folgenden Beispiel haben wir eine klassiche Klasse, welche aber  zwei generische Methoden beinhaltet. Den generischen
Typ einer Methode findet man noch vor dem Rückgabetyp in spitzen Klammern stehen. Im Fall von `addElement` sehen
wir den Typparameter `E`, der sich als Parameter des RÜckgabetyps findet, wie auch in den Parametern.

...
Wenn man jetzt meint, das `Void` und `void` sich wie `Integer` und `int` verhalten, der wird leider enttäuscht.
`Void` kann nicht `void` sein. Dafür aber `null`, und nur `null`

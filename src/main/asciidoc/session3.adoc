...
Funktionen, die nur einen Eingabeparameter haben. Ein simples Beispiel wäre eine Additionsfunktion
`const add = (x, y) => x + y`.
...
Neben anderen Lösungen, können wir hier aber auch das Currying benutzen, um die Fallentscheidung, welches Environment
genutzt werden soll, zentral in das `api`-Objekt auslagern.
...
NOTE: __Genau genommen ist das letzte Beispiel kein formal korrektes Currying, da wir anstatt einer neuen Funktion ein
        neues Objekt zurück geben.__


== function composition

Die Möglichkeit, Funktionen beliebig zu zu erstellen und zu übergeben, eröffnet ein paar interessante Wege, Funktionen
geschickt zu kombinieren und somit Logiken aufzutrennen und wiederverwendbar zu machen.

Das erste Beispiel definiert eine Datenstruktur, die zwei Werte (`first` und `second`) enthalten kann und auf die wir
mit zwei Funktionen (`selectFirst` und `selectSecond`) zugreifen können.

Zunächst definieren wir die `select`-Funktionen. `selectFirst` ist dadurch definiert, dass die Funktion einen ersten
Parmater `first` annimt und daraufhin eine weitere Funktion zurück gibt. Diese nimmt den `second`-Parameter entgegen
und gibt daraufhin `first` zurück, was noch im Scope der zweiten Funktion liegt.

[source,javascript]
----
include::{sourcedir}/deepdive/functional/FunctionalPatterns.js[tags=data_structure-select_first]
----

`selectSecond` ist analog zu `selectFirst` aufgebaut. Nur das hier der `second` Wert zurück gegeben wird.

[source,javascript]
----
include::{sourcedir}/deepdive/functional/FunctionalPatterns.js[tags=data_structure-select_second]
----
...
[source,javascript]
----
include::{sourcedir}/deepdive/functional/FunctionalPatterns.js[tags=data_structure-select_usage]
----
...
#[source,javascript]
#----
#include::{sourcedir}/deepdive/functional/FunctionalPatterns.js[tags=functional_list]
#----

#[source,javascript]
#----
#include::{sourcedir}/deepdive/functional/FunctionalPatterns.js[tags=validation]
#----
